...
UPLOAD_FOLDER = './input'
mydb = MySQLdb.connect(host='localhost',user='root',passwd='root',db='onlineexam')
conn = mydb.cursor()
app = Flask(__name__)
...
def finalevaluvation(text,qid,rollno):
        ...
        if total_score > 0.5:
                scoreforaquest=am
                status = "Pass"
        elif total_score > 0.3 and total_score<0.5 :
                scoreforaquest=am/2
                status = "Pass"
        elif total_score > 0.2 and total_score<0.3 :
                scoreforaquest=am/3
                status = "Pass"
        else:
                scoreforaquest=0.0
...
def evaluvation(rollno,qid):
        print("rollno==",rollno)
        fname=""
        cmd="SELECT fname FROM student_upload where rollno='"+rollno+"' and  qid='"+qid+"'"
        print(cmd)
        conn.execute(cmd)
        data=str(conn.fetchall())
        data=data.replace('(','')
        data=data.replace(')','')
        data=data.replace(',','')
        data=data.replace("'","")
        print("qid==",qid)
        path="input//"+rollno+"//"+qid+"//"+data
...
def segment_file():
        ...
        if request.method == 'POST':
                ...
                cmd="INSERT INTO student_upload(rollno,qid,fname) Values('"+str(uname)+"','"+str(qid)+"','"+str(filename)+"')"
                print(cmd)
                print("Inserted Successfully")
                conn.execute(cmd)
                mydb.commit()
